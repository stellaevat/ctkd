...
def compute_attention(q, k, d, mask, transpose=False, masked_val=1e5):
    attention = q @ k.transpose(-1, -2) / math.sqrt(2 * d)
...
def dskd_loss_fn(s_args, t_args, projectors, kl_temp):
    ...
    s_embeds = torch.cat([s_input_embeds, s_target_embeds], dim=-1)
    t_embeds = torch.cat([t_input_embeds, t_target_embeds], dim=-1)
    ...
    # Projection
    attn_mask = s_pad_mask.unsqueeze(-1) * t_pad_mask.unsqueeze(-1)
    ...
    s2t_attention = compute_attention(queries, keys, t_hiddens.shape[-1], attn_mask, transpose=True)
    ...
    t2s_logits_mask = ~(t2s_logits.isnan() | t2s_logits.isinf())
    s2t_logits_mask = ~(s2t_logits.isnan() | s2t_logits.isinf())

    t2s_ce_loss = (s_pad_mask * ce_loss_fn((t2s_logits * t2s_logits_mask).transpose(-1, -2), s_targets * s_pad_mask)).sum()

    t2s_kl_div = kl_div_fn((s_logits / kl_temp).log(), t2s_logits/kl_temp)
    t2s_kd_loss = (t2s_kl_div * t2s_true_mask.unsqueeze(-1) * t2s_logits_mask).sum()

    s2t_kl_div = kl_div_fn(s2t_logits.log(), t_logits)
    s2t_kd_loss = (s2t_kl_div * s2t_true_mask.unsqueeze(-1) * s2t_logits_mask).sum()
