...
def dskd_loss_fn(s_args, t_args, projectors, kl_temp):
    s_lm_weights, s_targets, s_hiddens, s_logits, s_input_embeds, s_target_embeds, s_mask = itemgetter(
        "lm_weights", "targets", "hiddens", "logits", "input_embeds", "target_embeds", "mask"
    )(s_args)

    t_lm_weights, t_targets, t_hiddens, t_logits, t_input_embeds, t_target_embeds, t_mask = itemgetter(
        "lm_weights", "targets", "hiddens", "logits", "input_embeds", "target_embeds", "mask"
    )(t_args)
    ...
    t2s_true_mask = s_mask * t2s_logits.argmax(-1).eq(s_targets)
    ...
    t2s_ce_loss = ce_loss_fn(t2s_logits * t2s_logits_mask, s_target_embeds)
